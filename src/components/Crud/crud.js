...
function CRUD(options) {
  ...
  const methods = {
    ...
    toAdd() {
      crud.resetForm()
      console.log('toAdd form',crud.form)
     ...
     */
    toEdit(data) {
      console.log('启动编辑', data)
      crud.resetForm(JSON.parse(JSON.stringify(data)))
      // crud.resetForm(JSON.parse(data))
    ...
    submitCU() {
      ...
      // console.log(crud.findVM('form').$refs['form'])
    ...
    selectionChangeHandler(val) {
      console.log('selectionChangeHandler', val)
      crud.selections = val
    },
    /**
     * 重置查询参数
     ...
     */
    resetForm(data) {
      console.log('resetForm',crud.form)
    ...
    resetDataStatus() {
      ...
      function resetStatus(datas) {
        datas.forEach(e => {
          ...
          console.log('重置数据状态', e)
          if (e.children) {
            resetStatus(e.children)
          } else if (e.childClass) {
            resetStatus(e.childClass)
          }
        })
      }
    ...
    selectAllChange(selection) {
      console.log('selectAllChange', selection)
    ...
    selectChange(selection, row) {
      ...
      if (selection.find(val => {
        ...
        } else if (row.childClass) {
          row.childClass.forEach(val => {
            // crud.getTable().toggleRowSelection(val, true)
            crud.findVM('presenter').$refs['table'].toggleRowSelection(val, true)
            selection.push(val)
            if (val.childClass) {
